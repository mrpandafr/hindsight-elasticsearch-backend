...
class ElasticsearchConnection(DatabaseConnection):
    ...
    async def fetchval(self, query: str, *args: Any, **kwargs: Any) -> Any:
        # Sole recognized statement: the universal liveness idiom. This is not
        # SQL translation — "SELECT 1" carries no data semantics, only "are
        # you alive?", which Elasticsearch answers with ping(). It makes the
        # engine's health_check() work with zero engine patch. Everything
        # else still raises.
        if re.match(r"\s*SELECT\s+1\s*;?\s*$", query, re.IGNORECASE):
            try:
