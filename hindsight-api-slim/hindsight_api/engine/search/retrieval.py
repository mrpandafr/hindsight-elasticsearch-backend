...
async def retrieve_semantic_bm25_combined(
    ...
    """
    # Elasticsearch backend: no SQL arms — dispatch to the native kNN/BM25
    # implementation (same signature, same return shape). Dispatch key mirrors
    # the dialect selection below (getattr for raw-asyncpg test connections).
    if getattr(conn, "backend_type", "postgresql") == "elasticsearch":
        from .retrieval_elasticsearch import retrieve_semantic_bm25_combined_es

        return await retrieve_semantic_bm25_combined_es(
            conn,
            query_emb_str,
            query_text,
            bank_id,
...
async def retrieve_temporal_combined(
    ...
    # Elasticsearch backend: window-filtered kNN entry points + native link
    # spreading (same formulas), no SQL. Same signature, same return shape.
    if getattr(conn, "backend_type", "postgresql") == "elasticsearch":
        from .retrieval_elasticsearch import retrieve_temporal_combined_es

        return await retrieve_temporal_combined_es(
            conn,
            query_emb_str,
            bank_id,
            fact_types,
...
async def retrieve_all_fact_types_parallel(
    ...
    # Elasticsearch backend: the configured retriever (LinkExpansion) is
    # SQL-based; when no explicit retriever was passed, substitute the native
    # one (seeds kNN -> ops.expand_semantic_causal / expand_entities ->
    # activation), which honours the same GraphRetriever contract.
    if getattr(pool, "backend_type", "postgresql") == "elasticsearch" and graph_retriever is None:
        from .retrieval_elasticsearch import ElasticsearchGraphRetriever

        retriever = ElasticsearchGraphRetriever()
